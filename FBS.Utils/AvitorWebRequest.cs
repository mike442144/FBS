...
namespace FBS.Utils
{
    ...
    public class AviatorWebRequest
	{
		/// <summary>
		/// 判断当前页面是否接收到了Post请求
		/// </summary>
        ...
        {
            string type = HttpContext.Current.Request.Files[0].ContentType;
            string[] checkSet = ConfigurationManager.AppSettings["ImgType"].Split('|');
            return checkSet.Contains(type);
        }
        ...
        {
            int length = 0;
            string fileLength = ConfigurationManager.AppSettings["ImgLength"];
            if (string.IsNullOrEmpty(fileLength))
                length = 5 * 1024 * 1024;
            else
                length = Convert.ToInt32(fileLength);

            return HttpContext.Current.Request.Files[0].ContentLength <= length;
        }
	}
}
